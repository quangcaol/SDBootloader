...
//#define FCLK_SLOW() { MODIFY_REG(SPI2->CR1,SPI_CR1_BR,SPI_BAUDRATEPRESCALER_128);}	/* Set SCLK = slow, approx 280 KBits/s*/
//#define FCLK_FAST() { MODIFY_REG(SPI2->CR1,SPI_CR1_BR,SPI_BAUDRATEPRESCALER_16); }	/* Set SCLK = fast, approx 4.5 MBits/s */

#define FCLK_SLOW() {  LL_SPI_SetBaudRatePrescaler(SD_SPI_HANDLE,LL_SPI_BAUDRATEPRESCALER_DIV128);}	/* Set SCLK = slow, approx 280 KBits/s*/
#define FCLK_FAST() { LL_SPI_SetBaudRatePrescaler(SD_SPI_HANDLE,LL_SPI_BAUDRATEPRESCALER_DIV32); }	/* Set SCLK = fast, approx 4.5 MBits/s */

#define CS_HIGH()	{LL_GPIO_SetOutputPin(SD_Detect_GPIO_Port, SD_Detect_Pin); while(0);}
#define CS_LOW()	{LL_GPIO_ResetOutputPin(SD_Detect_GPIO_Port, SD_Detect_Pin); while(0);}
...
{
	BYTE rxDat;
	spi_send_and_read(&dat, &rxDat, 1, 50);
...
)
{
	...
	recv_spi(0xFF); recv_spi(0xFF);			/* Discard CRC */
...
)
{
	...
	tx_spi(token);					/* Send token */
	if (token != 0xFD) {				/* Send data if token is other than StopTran */
		xmit_spi_multi(buff, 512);		/* Data */
		tx_spi(0xFF); tx_spi(0xFF);	/* Dummy CRC */

		resp = recv_spi(0xFF);				/* Receive data resp */
		...
		if (CardType & CT_SD2) {	/* SDC ver 2.00 */
			if (send_cmd(ACMD13, 0) == 0) {	/* Read SD status */
				...
				if (rcvr_datablock(csd, 16)) {				/* Read partial block */
					for (n = 64 - 16; n; n--) recv_spi(0xFF);	/* Purge trailing data */
